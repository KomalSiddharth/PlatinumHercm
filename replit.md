...
- **Progressive Unlock System (Health)**: Users must maintain 7/7 rating across ALL Health platinum standards for 7 consecutive days to unlock the ability to rate 7 in the main HRCM Health rating column. Features real-time unlock progress tracking with visual indicators (e.g., "3/7 days → unlock 7"), automatic streak reset if any standard drops below 7, and persistent unlock status stored in `user_hrcm_unlock_progress` table. Rating is capped at 6 until unlock condition is met.
...
- **Reporting & Analytics**: PDF export for reports, monthly progress analytics with charts, summary view, weekly progress badge, and a real-time team leaderboard.
...
- **User Interface**: Compact views with tooltips, popup editing, hover cards, and calendar history navigation. Clickable date navigation for calendar popup. Read-only historical data popup for checkpoint details.
