...
### UI/UX Decisions
The dashboard features a modern, polished design system centered on a vibrant brand gradient (Hot Pink #FF69B4 → Violet #7C3AED → Royal Blue #4169E1), creating an energetic yet professional aesthetic. Built with shadcn/ui components and the Inter font family, it provides seamless light/dark mode support with automatic system preference detection and localStorage persistence. The color palette uses semantic colors for intuitive UX: Emerald (#10B981) for success/growth, Amber (#F59E0B) for current week/caution, Soft Red (#EF4444) for urgent items, and Warm Gold (#FCD34D) for achievements. All colors meet WCAG AAA contrast requirements (4.5:1+ ratio). Design features include brand gradient navigation buttons with hover lift effects, enhanced card shadows with depth, smooth 0.3s transitions for theme switching, and generous spacing (24px sections, 16px cells, 12px buttons). The HRCM rating system is standards-based, scaled out of 10. Responsive mobile-first design uses CSS Grid/Flexbox, stacking tables on <768px viewports. Accessibility features include ARIA labels, brand primary focus rings, and optimized line-height (1.6) for readability.
