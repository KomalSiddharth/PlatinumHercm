...
- **Reporting & Analytics**: PDF export for reports, monthly progress analytics with charts based on platinum standards ratings (not checklist completion), summary view, weekly progress badge (platinum standards-based), and a real-time team leaderboard. Analytics graphs display data from the Progress column calculation using platinum standards: each HRCM area's progress = (Standards rated 7 ÷ Total standards) × 100%, averaged across all days in the selected period.
