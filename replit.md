...
- **Weekly Progress Calculation**: Weekly Progress badge displays the average progress across all 4 HRCM areas (Health, Relationship, Career, Money) for 7 consecutive days. Daily progress for each area is calculated using Average Rating Progress formula: (Average of all ratings ÷ 7) × 100%. This shows immediate progress and rewards improvement, while still encouraging perfect 7s for 100%. The weekly average is computed by averaging daily progress across the entire week, providing comprehensive insight into overall HRCM performance.
...
- **Reporting & Analytics**: PDF export for reports, monthly progress analytics with interactive charts (Bar Chart, Line Chart, Summary) based on platinum standards ratings (not checklist completion), summary view, weekly progress badge (platinum standards-based), and a real-time team leaderboard. Analytics graphs display data using the Average Rating Progress formula: each HRCM area's progress = (Average of all ratings ÷ 7) × 100%, averaged across all days in the selected period. This motivating formula shows progress immediately while encouraging perfect 7s for 100%. Fixed analytics endpoint to properly fetch and calculate progress data using correct database schema.
