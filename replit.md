...
- **Life Problems & Life Skill Map**: Google Sheets-powered course tracking (Sheet ID: 1na9ioh9uT8wxSkjTMxG61hUF75JxuSTF) with user-specific lesson completion tracking. Bold lines in Question column = Main courses with progress bars showing completion (X/Y lessons, Z%). Non-bold lines = Clickable lessons with interactive checkboxes for marking completion, linking to URLs from Answer column. Each lesson displays "10 pts". Lesson completions persist in database using courseVideoCompletions table. Supports collapsible course structure with nested subcategories and tree-based hierarchical view. Progress bars automatically update based on checked lessons.
