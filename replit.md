...
**Daily Auto-Copy & Real-Time Auto-Sync (Nov 6-7, 2025):**
- **Daily Auto-Copy Feature (RE-ENABLED Nov 7, 2025)**: ✅ When user views current day and no data exists, system automatically fetches previous day's data via `/api/hercm/previous-day/:date` endpoint, copies it to current day, and auto-saves. Enables seamless daily continuity (e.g., Thursday data → Friday, Friday → Saturday). **Critical Fix**: Each day's data is preserved as a separate database record using `(userId, weekNumber, dateString)` unique combination, ensuring Thursday's data remains visible in calendar while Friday gets a fresh copy. **User Control**: Auto-copy ONLY works when viewing current date (not when viewing history dates or in admin mode) - history dates show blank template if no data exists.
- **Real-Time Auto-Sync**: Automatic synchronization from Current Week to Next Week Target in real-time as user types. Mapping: Current Week Problems → Next Week Results, Current Week Feelings → Next Week Feelings, Current Week Beliefs → Next Week Beliefs/Reasons, Current Week Actions → Next Week Actions. Includes checkpoint checklists synchronization. **Universal Auto-Sync (Nov 7, 2025)**: Auto-sync now works on ALL dates (including past/history dates), not just today. Only disabled in admin view or when manual mode is active.
- **Update Button**: Located in Next Week Target header. Clicking "Update" clears all Next Week Target fields and checklists, disables auto-sync (sets `manualNextWeekMode = true`), and allows manual planning without auto-sync interference. **Universal Availability (Nov 7, 2025)**: Update button now available on ALL dates (including past/history dates), not just today. Only hidden in admin view.
...
- **Database UPSERT Logic**: `getHercmWeekByDate()` storage function checks by `(userId, weekNumber, dateString)` unique combination. Each calendar date maintains its own historical record with complete data independence.
- **Critical Data Corruption Fix (Nov 7, 2025)**: Fixed bug where auto-save operations (hover card edits, dialog close, Platinum Standards save) were missing `dateString` parameter, causing backend to default to today's date instead of selected calendar date. This resulted in data overwriting across different dates (e.g., editing 6th's data would overwrite 7th's data). All save operations now include `dateString: currentDateStr` to preserve date-specific data integrity. Affected functions: `handleHoverClose()`, `saveHoverEdit()`, `saveEditDialog()`, and Platinum Standards "Save & Close" button.
...
- **Course Tracker (Life Problems & Life Skill Map)**: **TEMPORARILY EMPTY (Nov 8, 2025)**: All courses removed from display - API endpoint returns empty array. User will provide new courses and lessons later. Empty state displays "No courses available at the moment." Features dark navy theme (#1a2942 background, border-2 border-primary/40, #0f1c2e inner content). **Overall Progress Bar**: Shows "Overall Progress" label with progress bar (no text count). Skill Map button in header uses teal-to-coral gradient matching header buttons (bg-gradient-to-r from-primary to-accent). Lesson completions persist across sessions in courseVideoCompletions database table. **Compact & Scrollable Layout**: Course tracking section features compact spacing and scrollable container with max-height of 500px (max-h-[500px] overflow-y-auto) for efficient space utilization on screen.
...
- **Platinum User Progress**: A team member dashboard viewer allowing users and admins to view **Week 1** HRCM data, daily rituals, emotional tracker, and earned badges of other team members, with search functionality. Always displays Week 1 data for consistency.
