...
- **Restored HRCM Dashboard UI** (Oct 2025): Restored previous design with header buttons layout - View History (outline), Save Week (pink/red), AI Auto-Fill Next Week (cyan), and Generate Next Week (pink with unlock date). History section displays all HRCM snapshots in collapsible cards showing week number, date, ratings, and problems with ability to navigate between weeks. AI Auto-Fill button calls `/api/hercm/ai-autofill-next-week` endpoint to generate Next Week suggestions. Generate Next Week button creates new week after 7-day restriction with unlock date display.
