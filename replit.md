...
- **Course Tracker**: Hardcoded course structure with 3 courses: "Manifest with Chakra by Mitesh Khatri" (19 lessons), "AI Course by Mitesh Khatri" (10 lessons), and "Depression To Celebration" (2 lessons). Features a dark navy theme, an overall progress bar, and persistent lesson completions. It utilizes a compact, scrollable layout. **Critical Backend Fix (Nov 8, 2025)**: Fixed inverted toggle logic where backend incorrectly treated `completed` parameter as desired state instead of current state.
