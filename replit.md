...
The frontend is built with React, Vite, Tailwind CSS, TanStack Query, and Wouter for routing. The backend uses Express.js with TypeScript (ES Modules), PostgreSQL with Drizzle ORM, and `connect-pg-simple` for session management. Authentication is session-based with `bcrypt` and implements role-based access control. Data persistence is managed via Drizzle ORM, Zod validation, and Drizzle Kit for migrations. The system employs UPSERT logic and a deduplication scoring mechanism for data integrity. Real-time synchronization is implemented for admin-defined "Platinum Standards" using `queryClient.refetchQueries()`. Course tracking is integrated with Google Sheets API, featuring a caching mechanism and advanced merging strategies for course data (e.g., DMP recordings). HRCM calendar and emotional tracker navigation utilize exact date matching for data retrieval. Admin panel implements performance optimizations with lazy-loaded queries (enabled only on active tabs), 30-second cache (`staleTime`) for heavy queries, and real-time polling (5-second intervals) for user feedback to enable instant updates without manual refresh.
