...
- **Daily Auto-Copy Feature**: When user views current day and no data exists, system automatically fetches previous day's data via `/api/hercm/previous-day/:date` endpoint, copies it to current day, and auto-saves. Enables seamless daily continuity (e.g., Thursday data → Friday, Friday → Saturday). **CRITICAL BUG FIX (Nov 6, 2025)**: Fixed historical data preservation bug where `createHercmWeek` was overwriting dateString with today's date, causing previous dates to appear blank in calendar. Now properly preserves dateString when provided, ensuring Thursday's data stays on Thursday when Friday gets auto-copied.
...
- **Historical Data Preservation**: All dates maintain their data in read-only calendar view. When Thursday's data is auto-copied to Friday, Thursday's original data remains visible in calendar (read-only). Each date's data persists independently with correct dateString.
