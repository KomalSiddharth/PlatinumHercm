...
- **Restored HRCM Dashboard UI** (Oct 2025): Restored previous design with header buttons layout - Save Week (pink/red), AI Auto-Fill Next Week (cyan), and Generate Next Week (pink). AI Auto-Fill button calls `/api/hercm/ai-autofill-next-week` endpoint to generate Next Week suggestions. Generate Next Week button is always unlocked and creates a new week instantly without any 7-day restriction.
- **Course Tracker Lesson Points System** (Oct 2025): Each lesson in Course Tracker now displays point values that contribute to the user's total points. Each lesson awards 10 points (default) when marked as completed. The Course Tracker header displays total lesson points earned with a teal-to-emerald gradient badge showing points and Book icon. Lesson points combine with ritual points in the global header for overall user score. Each lesson displays a small points badge (+10 pts) next to its title, with visual styling that adapts based on completion status (default variant for uncompleted, secondary for completed).
- **Unified Assignment Column** (Oct 2025): Assignment column redesigned as a single unified column in Next Week table, consolidating all Course Tracker lessons regardless of HRCM category. When users check any lesson in Course Tracker, it automatically appears with a checkbox in the unified Assignment column without requiring category selection. Removed per-HRCM assignment fields in favor of `unifiedAssignment` jsonb field. The unified column spans all 4 HRCM rows using rowspan for cleaner table layout. Endpoints: `/api/unified-assignment/add-lesson`, `/api/unified-assignment/toggle-lesson`, `/api/unified-assignment/remove-lesson`.
- **Weekly Progress Analytics Dialog** (Oct 2025): Clicking the weekly progress badge (e.g., "46% Weekly Progress") opens a detailed analytics dialog showing graphical progress bars for all 4 HRCM areas. Each area displays color-coded gradient progress bars (Health: red-orange, Relationship: blue-cyan, Career: purple-pink, Money: green-emerald), standards completion count, and current rating. Includes summary statistics showing total standards completed and average rating across all areas.
- **History Button Replacement** (Oct 2025): Replaced "Save Week" button with "History" button in HRCM table header. Clicking History opens the HRCM History Modal showing all previous week snapshots with edit history. Auto-save functionality continues to work on field blur, making manual save button unnecessary.
- **Enhanced Analytics with Multiple Chart Types** (Oct 2025): Weekly Progress Analytics dialog now features bar charts, line charts, and summary views with color-coded visualizations for all 4 HRCM areas. Users can toggle between weekly and monthly views, select specific months via calendar picker, and choose specific weeks for detailed analysis. Analytics endpoint `/api/analytics/progress` provides real-time data for graphical representations. Uses Recharts library for interactive visualizations.
- **Refined History Modal with Date Filtering** (Oct 2025): History modal redesigned to show only recent snapshots of Current Week and Next Week Target tables. Features calendar-based date picker and month selector for filtering historical data. Snapshots are grouped by date and time, displaying both Current Week (with Platinum Standards progress) and Next Week Target (with checklists for Results, Feelings, Beliefs, Actions). All historical data displayed in read-only format with disabled checkboxes.
